...
// IMPORTANT: Use memoryStorage — Vercel serverless has no writable filesystem
const upload = multer({
...
// ─────────────────────────────────────────────────────────────
//  POST /api/transcribe/image  →  Groq Vision (LLaMA 4 Scout)
// ─────────────────────────────────────────────────────────────
...
Output ONLY the extracted content — no commentary, no preamble.
If you cannot read any text, respond with: "No readable text found in image."`
    ...
    console.log(`✅ Image extraction done. ${cleaned.length} chars`);
...
router.post('/audio', upload.single('audio'), async (req, res) => {
  try {
    ...
    console.log(`🎙 Audio: ${req.file.originalname} (${req.file.size} bytes)`);
    ...
    console.log(`✅ Transcription done. ${transcript.length} chars`);
...
// ─────────────────────────────────────────────────────────────
//  Groq Whisper helper (pure buffer — no disk needed)
// ─────────────────────────────────────────────────────────────
async function groqWhisper(buffer, filename, mimeType, key) {
  return new Promise((resolve, reject) => {
    ...
    const body = Buffer.concat([filePart, rest]);
    ...
    const req = https.request({
      ...
      response.on('end', () => {
        try {
          ...
          if (json.error) return reject(new Error('Groq Whisper: ' + (json.error.message || JSON.stringify(json.error))));
          if (!json.text)  return reject(new Error('Groq returned no text. Response: ' + data.substring(0, 200)));
          resolve(json.text);
        } catch(e) {
